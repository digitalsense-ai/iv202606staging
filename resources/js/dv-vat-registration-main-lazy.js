        ...
        $.ajax({
          ...
          success: function (result) {
            ...
            if(result.status == 200)
            {
              ...
              //only view
              var enable = status;
              loadEnableDisableVatReturnsTabItems(client_id, null, vat_reg_main_id, enable);
...
// Update Excise Duty
  ...
  $(document).on('click', '.select-accountnos', function () {    
    ...
    $('li input[name="chk_accoutno[]"]:checked').each(function() {         
      var acc_no_data = $(this).data();
      var account_name = acc_no_data['acc_name'];
      var account_type = acc_no_data['acc_type'];
      var acc_reverse = ($(this).closest('li').find('input[type="checkbox"][name="acc_reverse[]"]:checked').length > 0) ? 1 : 0;
 ...
 console.log(selected_acc_no);
    ...
    $.each(selected_acc_no, function(key,value) { 
      ...
      if(value['acc_map_column'] == 'vat_sales' || value['acc_map_column'] == 'vat_purchases')
      {
        if(value['acc_auto_vat_check'] == 0)
          acc_auto_vat_check_text = 'Overview only';
        else if(value['acc_auto_vat_check'] == 1)
          acc_auto_vat_check_text = 'VAT check only';
        else if(value['acc_auto_vat_check'] == 2)
      ...
      var acc_map_column  = ((value['acc_map_column']) ? value['acc_map_column'] : '');
      var map_column = '';
      if(acc_map_column == 'net_sales')
        map_column = "Net Sales";
      else if(acc_map_column == 'vat_sales')
        map_column = "Output VAT (on Sales)";
      else if(acc_map_column == 'net_purchases')
        map_column = "Net Purchases";
      else if(acc_map_column == 'vat_purchases')
  ...
  $(document).on('change', '#formVatRegistrationMain #country', function () {      
    ...
    if($("#duty_defer_acc").is(':checked'))
    {
      $("#parent_dda_acc_no").show();
      $("#parent_dda_acc_limit").show();
    }
    else
    {
      $("#parent_dda_acc_no").hide();
      $("#parent_dda_acc_limit").hide();
    }
  ...
  $(document).on('click', '#formVatRegistrationMain #duty_defer_acc', function () { 
    if($(this).is(':checked'))
    {
      $("#parent_dda_acc_no").show();
      $("#parent_dda_acc_limit").show();
