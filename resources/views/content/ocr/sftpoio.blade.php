...
@endphp
    ...
    <div class="card analyzepdfsftpoio mt-4">      

      <!-- <h5 class="m-0 p-3">Synced Data's</h5> -->
      <div class="d-flex align-items-center gap-2 p-3">
        <h5 class="m-0">SFTP/OIO Data's</h5>
        {{--<span class="text-danger fs-6">
          <i class="bx bx-filter-alt me-1"></i>
          Select Client and Use Filter to check data
        </span>--}}

        <span id="btn-analyzepdfsftpoio-sales-invoice"  class="alert-primary text-end fs-tiny p-1 mx-2"></span>

        <div class="dropdown ms-auto w-px-300">
            <select
                id="select2OcrSftpOioClient"
                ...
                @foreach($sftpclients as $index => $clientName)
                    <option
                        value="{{ $clientName }}">
                        {{ $clientName }}
                    </option>
      ...
      <div class="card-header p-0"> 
        <div class="tab-pane fade show active" id="navs-analyzepdfsftpoio-sales-invoice" role="tabpanel"> 
            <div class="dt-sftpoio-filter text-end align-middle">                
            </div>              
            <div id="top-scroll-navs-analyzepdfsftpoio-sales-invoice" class="dt-top-scroll">
                <div class="dt-top-scroll-inner"></div>
            </div>
             <table class="datatables-analyzepdfsftpoio datatables-sales-invoice-analyzepdfsftpoio table">         
...
{{--
          ...
          <div class="col-md-8">
            <ul class="nav nav-tabs card-header-tabs mx-0" role="tablist">
              ...
              @for ($i = 0; $i < $end_table; $i++)
                ...
                <li class="nav-item">
                  <button type="button" id="btn-analyzepdfsftpoio-{{ $tab_div_id }}" class="nav-link {{ ($i === 0) ? 'active' : '' }}" role="tab" data-bs-toggle="tab" data-bs-target="#navs-analyzepdfsftpoio-{{ $tab_div_id }}" aria-controls="navs-analyzepdfsftpoio-{{ $tab_div_id }}" aria-selected="{{ ($i === 0) ? 'true' : 'false' }}"> {{ ucfirst($tab_div_name) }}<span class="alert-{{ $tab_span_class_name }} text-end fs-tiny p-1 mx-2"></span></button>                 
                </li>
              @endfor                
            </ul>
          </div>  
         
          <div class="col-md-4 dt-analyzepdfsftpoio-export text-end">
            @for ($i = 0; $i < $end_table; $i++)
                @php
                  $tab_div_id = '';            
                  if ($i === 0)
                ...
                <div class="{{ $tab_div_id }}-analyzepdfsftpoio-export {{ ($i === 0) ? '' : 'd-none' }}"></div>
            @endfor
          </div> 
          ...
          <div class="card shadow-none px-0">
            ...
            <div class="tab-content p-0 pb-4">
              ...
              @for ($i = 0; $i < $end_table; $i++)
                ...
                </div><!--/ navs-analyzepdfsftpoio-{{ $tab_div_id }}--> 
              @endfor
            </div>
    ...
    <!-- Loading overlay -->
    <div id="ocr-sftpoio-loading-overlay" class="ocr-loading-overlay d-none">
