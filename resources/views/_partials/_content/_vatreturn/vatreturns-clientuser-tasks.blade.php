...
<div class="accordion-item card sort-item" data-country="{{ $vatreg->country }}" data-vat_reg_main_id="{{ $vatreg->vat_reg_main_id }}" data-index="{{ $vatreg->statusorder }}" data-range="{{ ($vatreg->frequency > 1) ? (\Carbon\Carbon::parse($vatreg->service_start)->format('Y-m') . '***' . \Carbon\Carbon::parse($vatreg->service_start)->addMonth(($vatreg->frequency-1))->format('Y-m')) : (\Carbon\Carbon::parse($vatreg->service_start)->format('Y-m')) }}" {{ isset($vatreg->show) ? (($vatreg->show) ? 'data-all=true' : 'data-all=false') : '' }} {{ (count($vatreturns) == 0) ? 'data-reload=true' : '' }}>
          ...
          <tbody>
            <tr>              
              ...
              <td class="border-bottom-0 p-0">
                ...
                <span class="badge rounded-pill bg-label-{{ ($vatreg->vatregmain->product_type == 3 || $vatreg->vatregmain->product_type == 5) ? 'danger' : 'primary' }}">{{ $product_type_name }}</span>  
              </td>
              ...
              <td class="border-bottom-0 p-0 pe-3 text-end" id="total-tax-top-{{ $vat_reg_id }}">{{ $currencyFormatter->format($totalvat) . ' ' . (($currencySymbol) ? $currencySymbol : $currencycode) }}</td>              
    ...
    <!-- <div id="accordionStyleAllTasks-{{ $vat_reg_id }}" class="accordion-collapse collapse {{ ($vatreg->is_disregard) ? 'disabled' : '' }}" data-bs-parent="#accordionStyleAllTasks">
