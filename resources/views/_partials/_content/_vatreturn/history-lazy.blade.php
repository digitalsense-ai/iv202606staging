          ...
          <li class="timeline-item timeline-item-{{ ($timeline['role'] == 'team-user') ? 'success' : $timeline['color'] }} mb-md-4 mb-5 {{$timelineClass . $timeline['direction']}}">
            ...
            <div class="timeline-event card p-0" data-aos="fade-left">
              <div class="card-header d-flex justify-content-between align-items-center flex-wrap">
                ...
                <div class="{{ (isset($timeline['additional_subject'])) ? '' : 'd-flex' }} align-items-center meta {{ (isset($timeline['additional_subject'])) ? 'w-60 text-end' : '' }}">
                  <h6 class="mb-0 {{ (isset($timeline['additional_subject'])) ? 'w-100 text-end' : '' }}">{{ isset($timeline['reminder_action_name']) ? 'Reminder ' : '' }}{{ $timeline['subject'] }}</h6>
                  @if(isset($timeline['additional_subject']))
                    <span class="small w-100 text-end">{{ $timeline['additional_subject'] }}</span>
                  @endif
              ...
              <hr class="my-0 mx-4 {{ (isset($timeline['filetype'])) ? (($timeline['filetype'] == 'pivs' || $timeline['filetype'] == 'documents' || $timeline['filetype'] == 'importvatfiles' || $timeline['filetype'] == 'comments') ? 'border-bottom' : '') : '' }}">     
              <div class="card-body">   
                {!! isset($timeline['reminder_action_name']) ? '<p class="mb-2">' . $timeline['reminder_action_name'] . '</p>' : '' !!}
                <p class="mb-2">{!! $timeline['message'] !!} {!! (isset($timeline['payment_date'])) ? '<br> Payment Date: ' . \Carbon\Carbon::parse($timeline['payment_date'])->format('d-m-Y') : '' !!} {{ (isset($timeline['filetype'])) ? (($timeline['filetype'] == 'pivs' || $timeline['filetype'] == 'importvatfiles' || $timeline['filetype'] == 'cash-account-statement' || $timeline['filetype'] == 'duty-deferment-account') ?  \Carbon\Carbon::parse('01-'.$timeline['monthyear'])->format('F Y') : '') : '' }}</p>  

                @if(isset($timeline['filetype']))
                  @if($timeline['filetype'] == 'pivs' || $timeline['filetype'] == 'documents')
                    @php
                    /*
                    <i class="bx bx-paperclip cursor-pointer"></i> {{ ($timeline['file_name'] == null) ? 'No PIVS uploaded' : $timeline['file_name'] }}<br/>
                    */
                    @endphp
                  @else
