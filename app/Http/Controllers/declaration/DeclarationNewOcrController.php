...
use App\Jobs\RematchOcrComInvoicesChunkJob;
...
class DeclarationNewOcrController extends Controller
{  
    public $authUser;
    public $clientIds;
    ...
    public function index(Request $request, $vat_reg_id)
    {
      ...
      // {          
        ...
        $vatreg = VATRegistration::with('vatregmain')->where('id', $vat_reg_id)->first();
        ...
        if (!$vatregmain->ocr_sync) {
            return abort(403, 'OCR Sync not enabled for this client.');
        }

        if (!$fetch_period_from) {
            return abort(403, 'No new Declaration is available for this VAT period.');
        }

        if($show)                          
        {
          //$declarations = $this->reloadDeclarationsNewOcr($vat_reg_id, true);   
          $declarations = $this->reloadDeclarationsNewOcr($vat_reg_id);
          ...
          return view('content.declaration.index-new-ocr', 
            [
    ...
    public function startRematchOcrComInvoices($client_id)
    {
        try {            
            // Get IDs in chunks so we don't load everything into memory
            $invoiceIds = ImportReconciliationComInvoices::query()
                ->whereHas('vatreg.client', function ($query) use ($client_id) {
...
// ]);
            ...
            if ($invoiceIds->isEmpty()) {
                return response()->json([
                    'success' => true,
                    'message' => 'No invoices need rematching.',
                    'batch_id' => null,
                    'completed' => true,
                ]);
            }
            ...
            // 500 invoices per job
            foreach ($invoiceIds->chunk(500) as $chunk) {

                $jobs[] = new RematchOcrComInvoicesChunkJob(
                    $client_id,
            ...
            $batch = Bus::batch($jobs)
                ->name('OCR Commercial Invoice Rematching - Client ' . $client_id)
                ->onQueue('ocrrematch')
                ->allowFailures()
                ->dispatch();
            ...
            return response()->json([
                'success' => true,
                'message' => 'Rematching is in progress.',
                'batch_id' => $batch->id,
            ...
            Log::error('Failed to start OCR rematching', [
                'client_id' => $client_id,
                'error' => $e->getMessage(),
            ]);
            ...
            return response()->json([
                'success' => false,
                'message' => 'Unable to start rematching.',
            ], 500);
        }
    }

    public function rematchOcrComInvoicesStatus(Request $request, $batch_id)
    {
        ...
        return response()->json([
            ...
            'declarations' => ($batch->finished()) ? $this->reloadDeclarationsNewOcr($vat_reg_id) : [],
        ]);
    }

    public function reloadDeclarationsNewOcr($vat_reg_id, $auto_rematch = false)
    {    
        try
        {
          //sleep(2);
          ...
          if($auto_rematch)
          {
            $rematch = $this->commonClass->rematchOcrComInvoices($client_id);
            ...
            if($vatreg->country == 'NO')
            {
              ...
              $cominvoices = ImportReconciliationComInvoices::with(['vatreg', 'vatreg.client'])                 
                  ->where('vat_reg_id', $vat_reg_id)
                  ->whereDoesntHave('relationmatchno', function ($query) {
                      $query->whereColumn('com_invoice_id', 'dv_import_reconciliation_com_invoices.id');
                  })
              ...
              {  
                ...
                $salesinvoices = ImportReconciliationSalesInvoices::whereNotNull('ocr_pdf_id')->where('com_invoice_id', $com_invoice_id)->get();
                  ...
                  foreach($salesinvoices as $salesinvoice)
                  {
                    ...
                    $this->commonClass->addLog($this->authUser, 'importreconcilation-sales-invoice-relation-rematch', 
                      [
                        ...
                        'Re-matched OCR' => "Relation match for sales invoice <b><u>" . $salesinvoice->invoice_no . "</u></b> changed from <b><u>" . $previous_cominvoiceid . "</u></b> to <b><u>" . $com_invoice_id . "</u></b><br>"       
                      ]
                    );
                  }
                }           
    ...
    /* -- GET /declaration-new-invoice/{vat_reg_id}/ocr-refresh -- */
    public function refreshOcr($vat_reg_id)
    {    
        try
        {
          ...
          if($vatreg)
          {            
            $from = 'specific-ocr-refresh';
         
            //$data = $this->commonClass->loadImportReconciliationDatasFromAzureDb($this->authUser, $vatreg, $from);
            $data = $this->commonClass->loadImportReconciliationDatasFromNewOcr($this->authUser, $vatreg, $from);
            ...
            //$vatreg = $this->reloadDeclarationsNewOcr($vat_reg_id);   
        ...
        {         
          ...
          $this->commonClass->addLog($this->authUser, 'error-log', 
            [
              'status' => 'Error',
              'controller' => 'Declaration New OCR Controller',
              'method' => 'refreshOcr',
    ...
    /* -- GET /declaration-new-invoice/{vat_reg_id}/ocr-status -- */
    public function refreshOcrStatus($vat_reg_id)
    {
      try
      {                    
        $pending_jobs = DB::table('jobs')
                          ->where('queue', 'ocrpdfsyncinvoices')                          
                          ->count();
        ...
        {          
            //$result['declarations'] = $this->reloadDeclarationsNewOcr($vat_reg_id, true); 
            
            //return response()->json(['status' => 'completed', 'failed' => false, 'result' => $result]);           

            $vatreg = VATRegistration::with(['client'])                                   
                          ->where('id', $vat_reg_id)
                          ->first();

            return response()->json(['status' => 'completed', 'failed' => false, 'clientId' => $vatreg->client_id]);           
        }        
      }      
      catch (\Exception $e) 
      {           
    ...
    /* --end GET /declaration-new-invoice/{vat_reg_id}/ocr-status -- */
    ...
    //POST declaration-new-invoice/$invoice_id/disregard (BOTH COMMENT & DISREGARD/ENABLE/RETAIN)
      ...
      { 
        ...
        foreach (explode(',', $selected_invoice_ids) as $invoice_id)
        {
          ...
          else if($invoice_name == 'sales')
          {
            ...
            $invoice = ImportReconciliationSalesInvoices::whereNotNull('ocr_pdf_id')->where('id', $invoice_id)->first();       
    ...
    //DELETE declaration-new-invoice/$invoice_id/deletecomment
    ...
    /* -- POST declaration-new-invoice/$invoice_id/rematch  -- */ 
      ...
      {   
        ...
        foreach (explode(',', $selected_invoice_ids) as $key => $invoice_id)
        {
          ...
          if($invoice_name == 'com')
          {
            ...
            $invoice->rematch_ocr_com_invoice_id = $rematch_invoice_id;     
            $invoice->save(); 
            ...
            {     
              ...
              $salesinvoices = ImportReconciliationSalesInvoices::whereNotNull('ocr_pdf_id')->where('com_invoice_id', $rematch_invoice->id);
              if($salesinvoices)
              {                                
                  $update_sales = ImportReconciliationSalesInvoices::whereNotNull('ocr_pdf_id')->where('com_invoice_id', $rematch_invoice->id)
    ...
    /* --end POST declaration-new-invoice/$invoice_id/rematch  -- */ 

    /* -- DELETE declaration-new-invoice/$invoice_id/rematch  -- */ 
    ...
    /* --end DELETE declaration-new-invoice/$invoice_id/rematch  -- */ 
   
    /* -- DELETE declaration-new-invoice/$invoice_id  -- */ //(DELETE PERMANENTLY)
    ...
    /* --end DELETE declaration-new-invoice/$invoice_id  -- */ 

    /* -- POST declaration-new-invoice/$invoice_id/move  -- */ 
    ...
    /* --end POST declaration-new-invoice/$invoice_id/move  -- */    

    /* -- GET declaration-new-invoice/$invoice_id/edit  -- */ 
    ...
    /* --end GET declaration-new-invoice/$invoice_id/edit  -- */ 

    /* -- POST declaration-new-invoice/$invoice_id/edit  -- */ 
    ...
    /* --end POST declaration-new-invoice/$invoice_id/edit -- */

    /* -- POST declaration-new-invoice/$invoice_id/refresh (BOTH COM. INVOICE/SALES INVOICE)-- */
    public function refreshSpecificData(Request $request, $invoice_id)
    {
      ...
      {       
        ...
        $log_name_text_suffix = 'specific-invoice-ocr-refresh';
        if($invoice_name == 'com')
        {
          $invoice_name_text = 'Commercial Invoices';
          $log_name_text = 'com-invoice';
          ...
          if($invoice->data_from === 'ivf' && $invoice->rematch_ocr_com_invoice_id)
          {
            $invoice = ImportReconciliationComInvoices::where('id', $invoice->rematch_ocr_com_invoice_id)->first();
          } 
        }
        else if($invoice_name == 'sales')
        {
        ...
        if(stripos(strtolower($request->invoice_no), ", ") !== false)
        {
          ...
          {
            $from = 'specific-invoice-ocr-refresh';
            ...
            $data = $this->commonClass->loadImportReconciliationDatasFromNewOcr($this->authUser, $vatreg, $from, str_replace(['SPG-', '-NO'], '', $invoice_no), $ocr_pdf_id);
          }
        } 
        else
        {
          ...
          $data = $this->commonClass->loadImportReconciliationDatasFromNewOcr($this->authUser, $vatreg, $from, str_replace(['SPG-', '-NO'], '', $request->invoice_no), $ocr_pdf_id);
        }       
    ...
    /* --end POST declaration-new-invoice/$invoice_id/refresh (BOTH COM. INVOICE/SALES INVOICE)-- */
    
    /* --POST declaration-new-invoice/$vat_reg_id/convert (BOTH COM. INVOICE/SALES INVOICE)-- */
    ...
    /* --end POST declaration-new-invoice/$vat_reg_id/convert (BOTH COM. INVOICE/SALES INVOICE)-- */
    ...
    /* -- POST declaration-new-invoice/$invoice_id/unmatch (COM. INVOICE)-- */
    ...
    /* -- POST declaration-new-invoice/$invoice_id/move-file  -- */ 
      ...
      {          
        ...
        foreach ($arr_selected_invoices as $invoice_no)
        {
          if($invoice_name == 'sales')
          {
            ...
            $sales_invoices = ImportReconciliationSalesInvoices::whereNotNull('ocr_pdf_id')->where('invoice_no', $invoice_no)->get();    
...
//           dd("some are not mvoed", $arr_selected_invoices, $moved);
    ...
    /* --end POST declaration-new-invoice/$invoice_id/move-file  -- */ 
}
