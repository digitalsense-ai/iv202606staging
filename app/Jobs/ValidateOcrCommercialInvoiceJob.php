...
class ValidateOcrCommercialInvoiceJob implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if ($this->manual || $this->searchSave) 
        {
            ...
            $mapped = CustomComInvoiceMapper::map($result, $this->clients, true, $invoice->created_at, $this->manual, $this->searchSave);
        }
        else
            $mapped = CustomComInvoiceMapper::map($result, $this->clients, true, $invoice->created_at);
// Log::info('Mapped', [
        ...
        { 
            ...
            {       
                ...
                if($invoice->no_of_attempts <= 2)
                {
                    ...
                    $ocrAnalyzeService->analyze($this->ocrProgressKey, $this->clients, [$fullPath], $folder, $batchId, null, $prevCaptures);
